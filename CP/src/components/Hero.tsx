...
const Hero = () => {
    return (
        <div>
            <div className="grid grid-cols-3 mt-10 px-8">
                {/* left div col */}
                ...
                {/* right div below*/}
                <div className="flex flex-col w-[880px] h-[982px] pr-[32px] bg-white rounded-2xl">
                    <div className="flex flex-row justify-between">
                        ...
                        <div className="flex items-center p-4 gap-4 ">
                            <p className="text-sm font-medium leading-snug text-[#1D4ED8]">Qualified</p>
                            <img src={Hline} alt="" />
                            <div className="flex flex-row gap-2 w-[65px] items-center justify-center">
                                <p className="text-[#0A0A0A] text-sm font-normal leading-snug">Task</p>
                                <img src={taskNum} alt="" />
                                {/* <p className="text-center bg-[#F7F8FD] rounded-[40px] text-[10px] font-normal leading-snug">25</p> */}
                            </div>
                            <img src={Hline} alt="" className="mr-4" />
                            <div className="flex flex-row gap-2 w-[65px] items-center justify-center ">
                                <p className="text-[#0A0A0A] text-sm font-normal leading-snug ml-4">Disqualified</p>
                    ...
                    <div>
                        {cardData.map((card, index) => (
                            <div key={index} className="flex flex-row h-[143px] gap-2 ">
                                ...
                                <div className="py-4">
                                    <div className="flex flex-row h-[111px] w-[538px] gap-6">
                                        ...
                                        <div className="flex flex-col justify-start">
                                            <p className="mb-2 text-sm font-semibold text-black">{card.name}</p>
                                            <p className="mb-2 text-[10px] font-medium text-black">{card.location}</p>
                                            <p className="text-black text-[10px] font-light">{card.education}</p>
                                            <div className="flex flex-row gap-2">
                                                {card.tags.map((tag, i) => (
                                                    <p key={i} className="text-[#1D4ED8] text-[8px] ">{tag}</p>
                                                ))}
                                            </div>
                                            <div className="flex flex-row gap-2 mt-2">
                                                {card.cities.map((city, i) => (
                                                    <p key={i} className="text-[8px] rounded-2xl justify-center py-0.5 text-[#036F91] bg-slate-50 h-4 px-2.5 font-medium">{city}</p>
                                                ))}
                                            </div>
                                        </div>
                                    </div>
